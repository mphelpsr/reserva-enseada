...
# (Princípio VII): consome `booking.transferred` e `booking.cancelled`,
# publicados pelo módulo booking (specs/002-booking/tasks.md, T053-T055).
# É suporte de infraestrutura para T059 (tasks.md) — o consumidor/lógica de
# aplicação entra só na Fase 3.4, depois que o booking tiver implementado T055
# e o payload do evento estiver acordado entre os dois lados.
#
...
# Ativar depois que specs/002-booking T054/T055 estiverem em produção.

variable "booking_topics_deployed" {
  description = "Se true, assina a fila nos tópicos SNS booking.cancelled/booking.transferred (precisam já existir no state do módulo booking)"
  type        = bool
  default     = false
}
