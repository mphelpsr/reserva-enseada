...
- **T059c** Publisher SNS do tópico `vessel.recebedor.changed` — **nova tarefa, criada em 2026-07-12** durante a implementação da Fase 3.3 do booking (ver "Contrato da Saga" em `plan-vessel-management.md`): o booking precisa do `payment_recebedor_id` do proprietário pra montar o split de pagamento no Pagar.me (FR-015 do spec-booking) e não há chamada síncrona entre módulos pra buscar isso sob demanda. Payload: `{vesselId: string, recebedorId: string}` — publicado uma vez POR EMBARCAÇÃO do proprietário (mesma convenção de `vessel.seatlimit.changed`), já que `payment_recebedor_id` vive em `Owner` (por `ownerId`) mas o booking organiza tudo por `vesselId`. **Bloqueio real**: hoje não existe nenhum caso de uso neste módulo que ESCREVA `payment_recebedor_id` — T019 (`PaymentRecebedorGateIntegrationTest`) só lê, o cadastro em si foi registrado desde o início como onboarding externo do Pagar.me, fora do escopo do módulo. Esta tarefa portanto depende de primeiro existir um ponto de escrita real (ex.: `SetPaymentRecebedorIdUseCase` ou equivalente) antes de fazer sentido publicar o evento — avaliar ao iniciar se isso deve ser parte da mesma tarefa ou uma tarefa própria anterior.
