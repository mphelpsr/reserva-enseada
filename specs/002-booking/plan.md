...
| `vessel.recebedor.changed` | `{vesselId: string, recebedorId: string}` — **novo evento, adicionado em 2026-07-12** durante a implementação da Fase 3.3 deste módulo: sem ele, `ConfirmBookingUseCase` (T039) não tem como montar o split de pagamento no Pagar.me (FR-015) — não há chamada síncrona pra buscar `payment_recebedor_id` do vessel-management sob demanda. Réplica local mantida em `VESSEL#<vesselId>/RECEBEDOR`. **Ainda não publicado do lado vessel-management** (ver T059c em `tasks-vessel-management.md`, que por sua vez depende de um ponto de escrita para `payment_recebedor_id` que também não existe lá ainda) — a infraestrutura de consumo deste lado (domínio `VesselRecebedor`, repositório) já está pronta desde a Fase 3.3, só falta o consumidor SQS de fato (T049-T052) e o publisher do outro lado. |
