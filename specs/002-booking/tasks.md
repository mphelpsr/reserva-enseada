...
- **T053** SNS Publisher: `booking.confirmed` (disparado por T039)
- **T054** SNS Publisher: `booking.cancelled` (disparado por T040 ou T042)
- **T055** SNS Publisher: `booking.transferred` (disparado por T041) — **esta tarefa fecha a pendência T059 deixada em `tasks-vessel-management.md`**: o vessel-management precisa assinar este mesmo tópico (junto com `booking.cancelled`) para efetivar o passo final da Saga de cancelamento (mover a reserva pra outra embarcação, ou liberar o dia definitivamente). Ao implementar T055, validar o payload do evento junto com quem for implementar a tarefa equivalente do lado do vessel-management, para os dois lados combinarem o mesmo contrato.
...
- **T055 é a tarefa que fecha o fluxo de Saga coreografada entre os dois módulos.** Recomendo implementá-la em conjunto com quem for revisar a T059 pendente em `tasks-vessel-management.md`, já que o contrato do evento (`booking.transferred` / `booking.cancelled`) precisa ser o mesmo dos dois lados.
