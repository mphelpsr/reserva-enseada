...
- **T052b** SQS Listener: `vessel.recebedor.changed` — **nova tarefa, criada em 2026-07-12** (ver "Contrato da Saga" em `plan-booking.md`). **Implementado na Fase 3.4**: `OperatorEventsConsumer.handleRecebedorChanged` mantém a réplica local `VESSEL#<vesselId>/RECEBEDOR` (`VesselRecebedor`/`VesselRecebedorRepository`) usada por `ConfirmBookingUseCase` (T039) para montar o repasse. **Rename para `pixKey` concluído em 2026-07-12**: `VesselRecebedorChanged`/`VesselRecebedor`/`OperatorEventsConsumer`/`ConfirmBookingUseCase` já leem `pixKey` (payload `{vesselId, pixKey}`, publicado pelo vessel-management via T059c, implementada). **Ainda NÃO implementado**: `PagarmeClient.createOrderWithSplit` continua chamando o endpoint de split nativo do Pagar.me (`recipient_id`) como stand-in — a integração real com o provedor de split Pix (Transfeera/OpenPix) não tem contrato de API definido em nenhuma spec/plan, então não foi codificada (ver javadoc de `PagarmeClient`, seção "Desatualizado em relação ao modelo de pagamento vigente"). Cartão via Pagar.me continua funcionando normalmente; só o repasse ao proprietário está pendente do desenho real dessa integração.
